...
function buildRevisitSummary(baseSummary = '', bullets = []) {
  ...
  if (trimmedSummary && hasChanges) {
    ...
    return `After revisiting the recommendation ("${trimmedSummary}"), keep it intact but apply the change list to sharpen execution.${detail}`;
  }
  if (trimmedSummary) {
    return `After reviewing the recommendation ("${trimmedSummary}"), it remains solid and should stay the same; no changes are necessary.`;
  }
  if (hasChanges) {
...
function buildRevisitChanges(bullets = []) {
  ...
  return bulletList.map((item, index) => formatChangeSuggestion(item, index + 1)).filter(Boolean);
}
